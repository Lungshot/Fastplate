"""
Material Icons Browser Dialog
Dialog for browsing and selecting Google Material Design icons.
"""
...
from fonts.material_icons import get_material_icons_manager, MaterialIcon, ICON_STYLES
from ui.widgets.slider_spin import FocusComboBox


class MaterialIconButton(QPushButton):
    """Button displaying a single Material Icon."""

    icon_selected = pyqtSignal(object)

    def __init__(self, icon: MaterialIcon, parent=None):
...
class MaterialIconsDialog(QDialog):
    """Dialog for browsing and selecting Google Material Icons."""
    ...
    def __init__(self, parent=None):
        ...
        self._manager = get_material_icons_manager()
        self._icon_buttons = []
        self._current_style = 'baseline'

        self._setup_ui()
        self._load_icons()

    def _setup_ui(self):
        self.setWindowTitle("Select Icon (Google Material)")
        ...
        self._style_combo.addItem("Baseline", "baseline")
        self._style_combo.addItem("Outline", "outline")
        self._style_combo.addItem("Round", "round")
        self._style_combo.addItem("Sharp", "sharp")
        self._style_combo.addItem("Two-tone", "twotone")
    ...
    def _load_icons(self):
        """Load icons from Material Icons manager."""
        if not self._manager.load():
            self._search_edit.setPlaceholderText("Error: Could not load Material Icons data")
            return
    ...
    def _load_svg_previews(self):
        ...
        style = self._style_combo.currentData() or 'baseline'

        # Load first batch immediately
        for i, btn in enumerate(self._icon_buttons[:20]):
            svg = self._manager.download_svg(btn.icon.name, style)
            if svg:
        ...
        if len(self._icon_buttons) > 20:
            ...
            QTimer.singleShot(100, lambda: self._load_remaining_previews(20, style))

    def _load_remaining_previews(self, start_index: int, style: str):
        ...
        if end_index < len(self._icon_buttons):
            ...
            current_style = self._style_combo.currentData() or 'baseline'
            if current_style == style:
                QTimer.singleShot(50, lambda: self._load_remaining_previews(end_index, style))
        else:
    ...
    def _on_search(self, text):
        ...
        # Search
        results = self._manager.search(text, category, limit=200)
    ...
    def _on_style_changed(self, index):
        ...
        self._current_style = self._style_combo.currentData() or 'baseline'

        # Reload SVG previews with new style
        self._load_svg_previews()
    ...
    def _on_icon_selected(self, icon: MaterialIcon):
    ...
    def _update_selected_preview(self):
        ...
        style = self._style_combo.currentData() or 'baseline'
        svg = self._manager.download_svg(self._selected_icon.name, style)
    ...
    def _on_insert(self):
        ...
        if not self._selected_svg:
            self._selected_svg = self._manager.download_svg(self._selected_icon.name, style)
